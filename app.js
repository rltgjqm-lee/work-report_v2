...
function showPage(pageId) {
    ...
    if(pageId === 6) {
        ...
        if(savedUserSign) {
            const img = new Image();
            img.onload = () => userCtx.drawImage(img, 0, 0, userCanvas.width, userCanvas.height);
        ...
        if(savedDemandSign) {
            const img = new Image();
            img.onload = () => demandCtx.drawImage(img, 0, 0, demandCanvas.width, demandCanvas.height);
...
function toggleAccidentInput(show) {
    document.getElementById('accidentDetailRow').style.display = show ? 'flex' : 'none';
    document.getElementById('accidentActionRow').style.display = show ? 'flex' : 'none';
}

function getPage1Data() {
    return {
        org: document.getElementById('orgName').value.trim(),
        proj: document.getElementById('projectName').value.trim(),
        user: document.getElementById('userName').value.trim(),
        demand: document.getElementById('demandName').value.trim()
    };
}

function getPage1Errors(data) {
    let errors = [];
    if(!data.org) errors.push("(1) 기관명을 넣어 주세요");
    if(!data.proj) errors.push("(2) 참여사업명 넣어 주세요");
    if(!data.user) errors.push("(3) 참여자성명을 넣어주세요");
    if(!data.demand) errors.push("(4) 수요처명을 넣어 주세요");
    return errors;
}

async function saveConfigOnly() {
    const data = getPage1Data();
    const errors = getPage1Errors(data);
    ...
    localStorage.setItem('conf_org', data.org);
    localStorage.setItem('conf_proj', data.proj);
    localStorage.setItem('conf_demand', data.demand);
    localStorage.setItem('conf_user', data.user);

    document.getElementById('topProjName').innerText = data.proj;

    await customAlert("기관 정보가 안전하게 저장되었습니다.");
}

async function nextFromPage1() {
    const data = getPage1Data();
    const errors = getPage1Errors(data);
    ...
    const confirmMsg = `기관은 [${data.org}]\n참여사업명은 [${data.proj}]\n참여자 성명은 [${data.user}]\n수요처(서비스 대상자)는 [${data.demand}] 입니다.`;
    
    const isOk = await customConfirm(confirmMsg);
    if(isOk) {
        document.getElementById('dUser').innerText = data.user;
        showPage(2);
    }
}
...
function startNewLog() {
    ...
    document.querySelector('input[name="accidentAction"][value="업무수행"]').checked = true;
    document.getElementById('actTotalTime').innerText = "- 시간";
...
function updateHiddenTime(type) {
    ...
    const hidden = document.getElementById(type === 'start' ? 'actStart' : 'actEnd');
    if (ampm && h && m) {
...
async function savePage3() {
    ...
    if (startMin % 10 !== 0 || endMin % 10 !== 0) {
        await customAlert("시간은 10분 단위(00, 10, 20, 30, 40, 50분)로만 입력 가능합니다.");
        return;
    }
    ...
    if (date > todayStr) {
        ...
        if (endMinutes > currentMinutes) {
            await customAlert("현재 시간 이후(미래 시간)로 일지를 종료할 수 없습니다.");
            return;
        }
    }
...
async function nextFromPage3() {
    ...
    if (start && end) {
        ...
        const endMin = parseInt(end.split(':')[1]);
        if (startMin % 10 !== 0 || endMin % 10 !== 0) {
            errors.push("시간은 10분 단위(00, 10, 20, 30, 40, 50분)로만 지정해 주세요.");
        }
    }
...
async function savePage5() {
    ...
    if (accident === '유') {
        ...
        const action = document.querySelector('input[name="accidentAction"]:checked').value;
        await customAlert(`안전사고는\n[유] 이며\n\n안전사고 내용은\n[${detail}] 한 후\n\n업무는 [${action}] 하였습니다.`);
    } else {
...
async function saveLog() {
    ...
    const accidentAction = document.querySelector('input[name="accidentAction"]:checked').value;
    ...
    const log = {
        ...
        content, place, accident, accidentDetail, accidentAction,
        uSign, dSign,
    ...
    tx.oncomplete = async () => {
        isSavedOnPage6 = true;
        await customAlert("성공적으로 저장되었습니다.\n(보고서를 출력하시거나 '처음으로'를 눌러주세요)");
        loadList();
    };
}
...
async function goHomeFromPage6() {
    if(!isSavedOnPage6) {
        if(await customConfirm("아직 일지를 저장하지 않았습니다.\n저장하지 않고 '처음으로' 돌아가시겠습니까?\n(수정을 원하시면 '취소'를 누르세요)")) {
            showPage(2);
        }
    } else {
        showPage(2);
...
function loadList() {
    ...
    req.onsuccess = (e) => {
        ...
        logs = logs.filter(log => {
            ...
            return parseInt(y) === currentDisplayYear && parseInt(m) === currentDisplayMonth;
        });

        if(logs.length === 0) {
            // 빈 상태일 때 그리드 속성 때문에 깨질 수 있으므로 임시 해제
            listEl.style.display = 'block';
            listEl.innerHTML = "<div style='text-align:center; padding: 20px; color:#7f8c8d;'>이 달에 작성된 일지가 없습니다.</div>";
            return;
        } else {
...
async function exportReports() {
    await customAlert("엑셀과 PDF 파일이 동시에 다운로드됩니다.\n잠시만 기다려주세요.");
    generateExcel();
    setTimeout(() => {
        generatePDF();
...
function generatePDF() {
    ...
    req.onsuccess = async (e) => {
        ...
        // 1. 헤더 설정
        document.getElementById('pOrg').innerText = localStorage.getItem('conf_org') || "";
        document.getElementById('pProject').innerText = localStorage.getItem('conf_proj') || "";
        document.getElementById('pDemand').innerText = localStorage.getItem('conf_demand') || "";
        document.getElementById('pUser').innerText = localStorage.getItem('conf_user') || "";

        // 2. 바디(표) 그리기
        const tbody = document.getElementById('pRows');
        tbody.innerHTML = "";
        
        logs.sort((a,b) => a.timestamp - b.timestamp).forEach((log, i) => {
            let accText = log.accident;
            if(log.accident === '유') accText += `<br>(${log.accidentDetail} / ${log.accidentAction})`;

            let userImg = `<img src="${log.uSign}" style="height:35px; width:auto;">`;
            let demandImg = log.dSign ? `<img src="${log.dSign}" style="height:35px; width:auto;">` : "";
...
function generateExcel() {
    ...
    req.onsuccess = async (e) => {
        ...
        // 테이블 헤더 (병합 포함)
        sheet.mergeCells('A4:A5'); sheet.getCell('A4').value = '연번';
        sheet.mergeCells('B4:B5'); sheet.getCell('B4').value = '활동일';
        
        sheet.mergeCells('C4:E4'); sheet.getCell('C4').value = '활동시간';
        sheet.getCell('C5').value = '시작(00:00)';
        sheet.getCell('D5').value = '종료(00:00)';
        sheet.getCell('E5').value = '총시간';

        sheet.mergeCells('F4:F5'); sheet.getCell('F4').value = '활동내용';
        sheet.mergeCells('G4:G5'); sheet.getCell('G4').value = '활동장소';
        sheet.mergeCells('H4:H5'); sheet.getCell('H4').value = '안전사고 발생유무\n(사고내용, 조치내용)';
        sheet.mergeCells('I4:I5'); sheet.getCell('I4').value = '참여자\n서명';
        sheet.mergeCells('J4:J5'); sheet.getCell('J4').value = '확인자(수요처)\n서명';
        ...
        ['A2','F2','A3','F3','A4','B4','C4','F4','G4','H4','I4','J4','C5','D5','E5'].forEach(ref => {
            sheet.getCell(ref).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF0F0F0' } };
            sheet.getCell(ref).font = { bold: true };
        });

        // 컬럼 너비
        sheet.columns = [
            { width: 5 }, { width: 12 }, { width: 10 }, { width: 10 }, { width: 10 },
            { width: 25 }, { width: 15 }, { width: 20 }, { width: 12 }, { width: 12 }
        ];
        ...
        logs.sort((a,b) => a.timestamp - b.timestamp).forEach((log, i) => {
            let accText = log.accident;
            if(log.accident === '유') accText += `\n(${log.accidentDetail} / ${log.accidentAction})`;

            const currentRow = startRow + i;
            const row = sheet.getRow(currentRow);
            row.height = 40;
            row.values = [
                i+1, log.date, log.start, log.end, log.totalTime,
                log.content, log.place, accText, '', ''
            ];
...
function renderTimeSelects() {
    ...
    let hourOptions = '<option value="" disabled selected>시</option>' + hours12.map(h => `<option value="${h}">${h}</option>`).join('');
    let minOptions = '<option value="" disabled selected>분</option>' + mins.map(m => `<option value="${m}">${m}</option>`).join('');
...
window.onload = () => {
    ...
    if(o) {
        ...
        document.getElementById('demandName').value = localStorage.getItem('conf_demand') || "";
        document.getElementById('userName').value = localStorage.getItem('conf_user') || "";
        document.getElementById('dUser').innerText = localStorage.getItem('conf_user') || "";
