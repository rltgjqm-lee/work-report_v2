...
const distanceToSegmentM = (point: LatLng, start: LatLng, end: LatLng): number => {
  const metersPerDegreeLng = METERS_PER_DEGREE_LAT * Math.cos((point.lat * Math.PI) / 180);
  ...
  const projection = Math.min(
    1,
    Math.max(0, -(startX * segmentX + startY * segmentY) / segmentLengthSquared),
  );
...
export const distanceOutsidePolygonM = (point: LatLng, polygon: LatLng[]): number => {
  if (polygon.length === 0) return Infinity;
  if (isPointInPolygon(point, polygon)) return 0;

  let shortest = Infinity;
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    shortest = Math.min(shortest, distanceToSegmentM(point, polygon[j], polygon[i]));
  }
  return shortest;
};
