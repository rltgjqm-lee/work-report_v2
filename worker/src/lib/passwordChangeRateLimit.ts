...
export const isPasswordChangeLocked = async (db: DB, adminId: number): Promise<boolean> => {
  const rows = await db
    .select()
...
export const recordPasswordChangeFailure = async (db: DB, adminId: number): Promise<void> => {
  const rows = await db
    .select()
    .from(adminPasswordChangeAttempts)
    .where(eq(adminPasswordChangeAttempts.adminId, adminId));
  ...
  if (row) {
    ...
    await db.insert(adminPasswordChangeAttempts).values({ adminId, failCount, lockedUntil });
  }
};

export const clearPasswordChangeFailures = async (db: DB, adminId: number): Promise<void> => {
  await db
