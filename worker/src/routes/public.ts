...
// 참여자가 방금 자동 기록된 출근 시각을 스스로 고칠 때 쓴다 — 관리자용 PUT
// /api/attendance/:logId(사유 필수, 감사 기록 남김)와 달리 참여자 셀프 수정이라
// 사유/감사 기록 없이 participantId 소유 확인만 한다.
app.patch("/attendance/clock-in", async (c) => {
  const db = drizzle(c.env.DB);
  ...
  if (log.groupId) {
    ...
    if (group) {
      const nowMinutes = toMinutes(body.time);
      const earliest = toMinutes(group.shiftStart) - 30;
      const latest = toMinutes(group.shiftEnd) + 30;

      if (nowMinutes < earliest || nowMinutes > latest) {
        return c.json(
          {
            error: `근무 시간(${group.shiftStart}~${group.shiftEnd}) 전후 30분 이내로만 수정할 수 있습니다.`,
          },
          400,
        );
      }
  ...
  const clockIn = `${date}T${body.time}:00.000Z`;
  const result = await db
    .update(attendanceLogs)
    .set({ clockIn })
    .where(eq(attendanceLogs.id, log.id))
...
// 퇴근 시각 셀프 수정 — 기존 clock-out과 동일한 지각/조퇴 판정을 새 시각 기준으로 다시 계산한다.
app.patch("/attendance/clock-out", async (c) => {
  const db = drizzle(c.env.DB);
  const body = await c.req.json<{ participantId?: number; time?: string }>();
  if (!body.participantId || !body.time) {
    return c.json({ error: "참여자와 시각을 지정해주세요." }, 400);
  ...
  const clockOut = `${date}T${body.time}:00.000Z`;
  const totalMinutes = Math.floor(
  ...
  const result = await db
    .update(attendanceLogs)
    .set({ clockOut, totalMinutes, status, note })
    .where(eq(attendanceLogs.id, log.id))
    .returning();
