...
app.get("/", async (c) => {
  ...
  // 조 배정도 같이 내려준다 — 사업단 상세 화면(수요처 관리 탭)이 이 목록과 배정을
  // 따로 조회해서 프론트에서 다시 묶던 걸 없애기 위함. 수요처가 없으면 스킵.
  const demandSiteIds = rows.map((row) => row.id);
  const scheduleRows =
    demandSiteIds.length === 0
      ? []
      : await db
          .select({
  ...
  const rowsWithSchedules = rows.map((row) => ({
    ...row,
    schedules: scheduleRows.filter((schedule) => schedule.demandSiteId === row.id),
  }));
