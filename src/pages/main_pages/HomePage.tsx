...
const HomePage = ({ formData, onOpenAffiliation, onStartActivityLog }: HomePageProps) => (
  ...
  >
    <div className="h-[60px] flex-none flex items-center justify-start px-[clamp(16px,5vw,24px)]">
      <span className="relative top-1 text-[clamp(20px,4.5vw,22px)] font-extrabold text-text-strong">
        WORK-REPORT
      </span>
    </div>
    ...
    <div className="flex-1 flex flex-col px-[clamp(16px,5vw,24px)] py-[clamp(20px,6vw,36px)]">
      ...
      <div className="flex gap-[clamp(8px,3vw,12px)] mt-[clamp(28px,8vw,40px)]">
        <button
          ...
          className="flex-1 min-w-0 aspect-square bg-white border-[1.5px] border-border-faint rounded-[18px] px-[clamp(12px,4vw,16px)] py-[clamp(14px,4.5vw,18px)] flex flex-col items-start justify-center gap-3 cursor-pointer text-left shadow-[0_2px_8px_rgba(20,30,50,.05)]"
        >
          <img
