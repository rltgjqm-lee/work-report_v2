...
import SettingsPageLargeFont from "./SettingsPageLargeFont";
...
interface SettingsPageProps {
  ...
  onBack: () => void;
  // 홈 화면에서 켠 큰글씨 모드를 그대로 물려받는다 — 이 화면 자체엔 전환 버튼이 없다.
...
const SettingsPage = ({
  ...
  return (
    <>
      {isLargeFontMode ? (
        <SettingsPageLargeFont
          onBack={onBack}
          participantId={participantId}
          locationConsentAt={locationConsentAt}
          otaVersion={otaVersion}
          onOpenLocationGuide={() => setIsLocationGuideOpen(true)}
        />
      ) : (
