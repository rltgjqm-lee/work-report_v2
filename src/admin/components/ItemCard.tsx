...
const ItemCard = ({ title, badge, infoLine, tags, actions, size = "compact" }: ItemCardProps) => (
  ...
  >
    ...
    <div className="text-[12px] text-admin-text-faint mt-1.5">{infoLine}</div>
    {tags && tags.length > 0 && (
      <div className="flex flex-wrap gap-1.5 mt-2">
        {tags.map((tag) => (
          <span
            key={tag}
            className="text-[11.5px] font-semibold text-admin-status-pending-text bg-surface-page px-2.5 py-1 rounded-full"
          >
            {tag}
          </span>
        ))}
    ...
    <div className="flex items-center justify-end gap-2 mt-3.5 pt-3 border-t border-border-faint">
      {actions.map((action) => (
